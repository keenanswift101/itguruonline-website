...
const ADDONS = [
  ...
  { key: "websiteDesign" as const, label: "Website Design", desc: "Professional design via Swift Designz", price: "Quote provided" },
];
...
export function StepServiceSelection({ data, onNext, onBack }: StepCProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-[var(--text-primary)]">Section C — Service Selection</h2>
        <p className="mt-1 text-sm text-[var(--text-secondary)]">Choose your hosting package and any additional services.</p>
      </div>
              ...
              <button
                ...
                className={`text-left rounded-xl border-2 p-4 transition-all focus:outline-none focus:ring-2 focus:ring-primary-500 cursor-pointer ${
                  selected
                    ...
                    : "border-[var(--border-color)] bg-[var(--bg-primary)] hover:border-primary-300 dark:hover:border-primary-700"
                }`}
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className={`mt-0.5 h-5 w-5 rounded-full border-2 flex items-center justify-center shrink-0 ${
                    selected ? "border-primary-700 bg-primary-700" : "border-[var(--border-color)]"
                  }`}>
                    {selected && (
      ...
      {/* Add-ons */}
      <div>
        <p className="text-sm font-semibold text-[var(--text-primary)] mb-3">Add-on Services</p>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[var(--text-primary)]">{addon.label}</p>
                  <p className="text-xs text-[var(--text-secondary)]">{addon.desc}</p>
                </div>
      ...
      {/* Additional notes */}
      <div>
        <label className="block text-sm font-medium text-[var(--text-primary)] mb-1" htmlFor="additionalServices">
        ...
        <textarea
          ...
          className="w-full rounded-lg border px-3 py-2 bg-[var(--bg-primary)] text-[var(--text-primary)] border-[var(--border-color)] placeholder:text-[var(--text-secondary)] focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none text-sm"
        />
      </div>
