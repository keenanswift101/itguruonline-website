  ...
  return (
    <div ref={ref} className="relative shrink-0">
      <button
        ...
        className="flex h-10 items-center gap-1.5 rounded-lg border px-2 bg-[var(--bg-primary)] text-[var(--text-primary)] border-[var(--border-color)] focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
      >
      ...
      {open && (
        <ul
          role="listbox"
          className="absolute z-20 mt-1 max-h-64 w-56 overflow-y-auto rounded-lg border bg-[var(--bg-primary)] border-[var(--border-color)] py-1 shadow-xl"
        >
          {COUNTRY_CODES.map((c) => (
            <li key={c.code} role="option" aria-selected={c.code === value}>
              <button
                ...
                className={`flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-[var(--bg-surface)] ${
                  c.code === value ? "bg-[var(--bg-surface)]" : ""
                }`}
              >
                <FlagImg iso2={c.iso2} />
                <span className="text-[var(--text-primary)]">{c.country}</span>
                <span className="ml-auto text-[var(--text-secondary)]">{c.code}</span>
              </button>
            </li>
          ))}
        </ul>
