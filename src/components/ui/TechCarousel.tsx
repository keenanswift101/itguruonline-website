    ...
    <div className="relative flex flex-col gap-5 overflow-hidden py-4" aria-hidden="true">
      ...
      <div className="flex w-max animate-[marquee-left_30s_linear_infinite] gap-5">
        {scrollItems.map((item, i) => (
          <CarouselCard key={`r1-${i}`} label={item.label} icon={item.icon} />
        ))}
      </div>

      {/* Row 2 — scrolls right (offset items for variety) */}
      <div className="flex w-max animate-[marquee-right_35s_linear_infinite] gap-5">
        {[...scrollItems.slice(4), ...scrollItems.slice(0, 4)].map((item, i) => (
          <CarouselCard key={`r2-${i}`} label={item.label} icon={item.icon} />
        ))}
      </div>
    </div>
  );
